...
class ReportListRecyclerAdapter(
    ...
    private var reportslist: ArrayList<ReportsResponseModel.ProgressReport>
) :
    RecyclerView.Adapter<ReportListRecyclerAdapter.MyViewHolder>() {

    lateinit var clickedurl: String
    lateinit var linearLayoutManager: LinearLayoutManager
    lateinit var detailArray: ArrayList<ReportsResponseModel.ReportItem>
    ...
    override fun onBindViewHolder(holder: ReportListRecyclerAdapter.MyViewHolder, position: Int) {
        holder.title.text = reportslist[position].academicYear
        ...
        if (reportslist.size > 0) {
            detailArray = reportslist[position].reportData
